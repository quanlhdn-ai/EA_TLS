...
input group "--- Strong Level Settings ---"
input color StrongHighColor       = clrRed; 
...
double maj_strong_high = EMPTY_VALUE; datetime maj_strong_high_time = 0; int maj_strong_high_idx = -1; 
double maj_strong_low = EMPTY_VALUE;  datetime maj_strong_low_time = 0;  int maj_strong_low_idx = -1;  
...
int OnCalculate(const int rates_total, const int prev_calculated, const datetime &time[], const double &open[], const double &high[], const double &low[], const double &close[], const long &tick_volume[], const long &volume[], const int &spread[])
{
   ...
   if(prev_calculated == 0) {
      ...
      maj_strong_high = EMPTY_VALUE; maj_strong_high_time = 0; maj_strong_high_idx = -1;
      maj_strong_low = EMPTY_VALUE;  maj_strong_low_time = 0;  maj_strong_low_idx = -1;
      last_strong_high_update_time = 0; last_strong_low_update_time = 0;
      pending_strong_high_update = false; pending_strong_low_update = false;
      pending_weak_high_level = EMPTY_VALUE; pending_weak_low_level = EMPTY_VALUE;
      processed_weak_high_time = 0; processed_weak_low_time = 0;
   ...
   if (prev_calculated > 0 && rates_total > prev_calculated) {
      ...
      if(maj_strong_high_idx >= 0) maj_strong_high_idx += shift;
      if(maj_strong_low_idx >= 0)  maj_strong_low_idx += shift;
   }
   ...
   for(int i = bos_limit; i >= 0; i--)
   {
      ...
      if (major_trend == 1) {
         if (maj_extreme_high == EMPTY_VALUE || high[i] > maj_extreme_high) { 
            maj_extreme_high = high[i]; maj_extreme_high_time = time[i]; maj_extreme_high_idx = i; 
         }
      } else if (major_trend == -1) {
         if (maj_extreme_low == EMPTY_VALUE || low[i] < maj_extreme_low) { 
            maj_extreme_low = low[i]; maj_extreme_low_time = time[i]; maj_extreme_low_idx = i; 
         }
      }

      if (minor_trend == 1) { 
         if (min_extreme_high == EMPTY_VALUE || high[i] > min_extreme_high) { 
            min_extreme_high = high[i]; min_extreme_high_time = time[i]; min_extreme_high_idx = i; 
         } 
      } else if (minor_trend == -1) { 
         if (min_extreme_low == EMPTY_VALUE || low[i] < min_extreme_low) { 
            min_extreme_low = low[i]; min_extreme_low_time = time[i]; min_extreme_low_idx = i; 
         } 
      }
      ...
      if(i > 0)
      {
         if (major_trend == 0) {
            if (ActiveHigh.isActive && HAClose[i] > ActiveHigh.price) { 
                ...
                maj_strong_low = maj_prot_low; maj_strong_low_time = maj_prot_low_time; maj_strong_low_idx = maj_prot_low_idx;
                ...
                maj_strong_high = maj_prot_high; maj_strong_high_time = maj_prot_high_time; maj_strong_high_idx = maj_prot_high_idx;
         ...
         if (major_trend == 1 && maj_confirmed_extreme_high != EMPTY_VALUE &&
             ...
             double temp_d4_val = EMPTY_VALUE; datetime temp_d4_time = 0; int temp_d4_idx = -1;
             int anchor_idx = -1;
             ...
             if(anchor_idx != -1) {
                 for(int k = i; k <= anchor_idx; k++) {
                     if(majorSwingLow[k] != EMPTY_VALUE) { 
                         temp_d4_val = majorSwingLow[k]; temp_d4_time = time[k]; temp_d4_idx = k;
                         break;
                     }
                 }
             }
             ...
             if(temp_d4_val != EMPTY_VALUE) {
                 double temp_d5_val = EMPTY_VALUE; datetime temp_d5_time = 0; int temp_d5_idx = -1;
                 for(int k = i; k <= temp_d4_idx; k++) {
                     if(temp_d5_val == EMPTY_VALUE || low[k] < temp_d5_val) {
                         temp_d5_val = low[k]; temp_d5_time = time[k]; temp_d5_idx = k;
                     }
                 }
                 if(temp_d5_val != EMPTY_VALUE && temp_d5_val < temp_d4_val) {
                     maj_prot_low = temp_d5_val; maj_prot_low_time = temp_d5_time; maj_prot_low_idx = temp_d5_idx;
                     is_maj_prot_low_sweep = true;
                 } else {
                     maj_prot_low = temp_d4_val; maj_prot_low_time = temp_d4_time; maj_prot_low_idx = temp_d4_idx;
                     is_maj_prot_low_sweep = false;
                 }
             }
             ...
             if(anchor_idx != -1) {
                 for(int k = i; k <= anchor_idx; k++) {
                     if(majorSwingHigh[k] != EMPTY_VALUE) { 
                         temp_d4_val = majorSwingHigh[k]; temp_d4_time = time[k]; temp_d4_idx = k;
                         break;
                     }
                 }
             }
             ...
             if(temp_d4_val != EMPTY_VALUE) {
                 ...
                 for(int k = i; k <= temp_d4_idx; k++) {
                     if(temp_d5_val == EMPTY_VALUE || high[k] > temp_d5_val) {
                         temp_d5_val = high[k]; temp_d5_time = time[k]; temp_d5_idx = k;
                     }
                 }
                 if(temp_d5_val != EMPTY_VALUE && temp_d5_val > temp_d4_val) {
                     maj_prot_high = temp_d5_val; maj_prot_high_time = temp_d5_time; maj_prot_high_idx = temp_d5_idx;
                     is_maj_prot_high_sweep = true;
                 } else {
                     maj_prot_high = temp_d4_val; maj_prot_high_time = temp_d4_time; maj_prot_high_idx = temp_d4_idx;
                     is_maj_prot_high_sweep = false;
                 }
             }
         ...
         if (major_trend == 1 && pending_prot_low_update && prot_breakout_idx != -1) {
             ...
             for(int k = i; k < rates_total; k++) {
                 if (time[k] == prot_anchor_time) { anchor_idx = k; break; }
             }

             if (anchor_idx != -1 && prot_breakout_idx <= anchor_idx) {
                 for(int k = prot_breakout_idx; k <= anchor_idx; k++) {
                     if (majorSwingLow[k] != EMPTY_VALUE) {
                         d4_val = majorSwingLow[k]; d4_time = time[k]; d4_idx = k;
                         break;
                     }
                 }
                 ...
                 if (d4_val != EMPTY_VALUE) {
                     ...
                     for(int k = prot_breakout_idx; k <= d4_idx; k++) {
                         if (d5_val == EMPTY_VALUE || low[k] < d5_val) {
                             d5_val = low[k]; d5_time = time[k]; d5_idx = k;
                         }
                     }
                     if (d5_val != EMPTY_VALUE && d5_val < d4_val) {
                         maj_prot_low = d5_val; maj_prot_low_time = d5_time; maj_prot_low_idx = d5_idx;
                         is_maj_prot_low_sweep = true; 
                     } else {
                         maj_prot_low = d4_val; maj_prot_low_time = d4_time; maj_prot_low_idx = d4_idx;
                         is_maj_prot_low_sweep = false;
                     }
         ...
         if (major_trend == -1 && pending_prot_high_update && prot_breakout_idx != -1) {
             ...
             if (anchor_idx != -1 && prot_breakout_idx <= anchor_idx) {
                 for(int k = prot_breakout_idx; k <= anchor_idx; k++) {
                     if (majorSwingHigh[k] != EMPTY_VALUE) {
                         d4_val = majorSwingHigh[k]; d4_time = time[k]; d4_idx = k;
                         break; 
                     }
                 }
                 ...
                 if (d4_val != EMPTY_VALUE) {
                     ...
                     for(int k = prot_breakout_idx; k <= d4_idx; k++) {
                         if (d5_val == EMPTY_VALUE || high[k] > d5_val) {
                             d5_val = high[k]; d5_time = time[k]; d5_idx = k;
                         }
                     }
                     if (d5_val != EMPTY_VALUE && d5_val > d4_val) {
                         maj_prot_high = d5_val; maj_prot_high_time = d5_time; maj_prot_high_idx = d5_idx;
                         is_maj_prot_high_sweep = true; 
                     } else {
                         maj_prot_high = d4_val; maj_prot_high_time = d4_time; maj_prot_high_idx = d4_idx;
                         is_maj_prot_high_sweep = false;
                     }
         ...
         if (minor_trend == 1) { 
            ...
            if (is_min_ha_break_up) {
               ...
               if (last_min_low != EMPTY_VALUE && last_min_low_time <= time[i]) { 
                  d4_val = last_min_low; d4_time = last_min_low_time; d4_idx = last_min_low_idx; 
               }
               if (ActiveMinorLow.isActive && ActiveMinorLow.time < min_extreme_high_time) { 
                  d4_val = ActiveMinorLow.price; d4_time = ActiveMinorLow.time; d4_idx = ActiveMinorLow.idx;
               }
               if (d4_idx != -1) {
                   double d5_val = EMPTY_VALUE; datetime d5_time = 0; int d5_idx = -1;
                   ...
                   if (d5_val != EMPTY_VALUE && d5_val < d4_val) {
                       min_prot_low = d5_val; min_prot_low_time = d5_time; min_prot_low_idx = d5_idx;
                       is_min_prot_low_sweep = true;
                   } else {
                       min_prot_low = d4_val; min_prot_low_time = d4_time; min_prot_low_idx = d4_idx;
            ...
            if (is_min_ha_break_dn) {
               ...
               if (last_min_high != EMPTY_VALUE && last_min_high_time <= time[i]) { 
                  d4_val = last_min_high; d4_time = last_min_high_time; d4_idx = last_min_high_idx; 
               }
               if (ActiveMinorHigh.isActive && ActiveMinorHigh.time < min_extreme_low_time) { 
                  d4_val = ActiveMinorHigh.price; d4_time = ActiveMinorHigh.time; d4_idx = ActiveMinorHigh.idx; 
               }
               if (d4_idx != -1) {
                   double d5_val = EMPTY_VALUE; datetime d5_time = 0; int d5_idx = -1;
                   for(int k = i; k <= d4_idx; k++) {
                   ...
                   if (d5_val != EMPTY_VALUE && d5_val > d4_val) {
                       min_prot_high = d5_val; min_prot_high_time = d5_time; min_prot_high_idx = d5_idx;
                       is_min_prot_high_sweep = true;
                   } else {
                       min_prot_high = d4_val; min_prot_high_time = d4_time; min_prot_high_idx = d4_idx;
                       is_min_prot_high_sweep = false;
                   }
               }
            }
         } else { 
            if (ActiveMinorHigh.isActive && HAClose[i] > ActiveMinorHigh.price) { minor_trend = 1; min_extreme_high = high[i]; min_extreme_high_time = time[i]; min_extreme_high_idx = i; min_prot_low = last_min_low; min_prot_low_time = last_min_low_time; min_prot_low_idx = last_min_low_idx; is_min_prot_low_sweep = false; } 
            else if (ActiveMinorLow.isActive && HAClose[i] < ActiveMinorLow.price) { minor_trend = -1; min_extreme_low = low[i]; min_extreme_low_time = time[i]; min_extreme_low_idx = i; min_prot_high = last_min_high; min_prot_high_time = last_min_high_time; min_prot_high_idx = last_min_high_idx; is_min_prot_high_sweep = false; }
         }
         ...
         if (major_trend == 1 && maj_strong_low != EMPTY_VALUE && HAClose[i] < maj_strong_low) {
            ...
            if(ObjectFind(0, choch_name) < 0) {
               ...
               ObjectDelete(0, "IND_SMC_MAJOR_KEY_LEVEL"); ObjectDelete(0, "IND_SMC_MAJOR_KEY_LEVEL_lbl");
               ...
               latest_break_down_time = time[i]; latest_break_down_idx = i; latest_break_down_level = maj_strong_low;
               ...
               if (ActiveLow.time == maj_strong_low_time) ActiveLow.isActive = false; 
               ...
               maj_strong_low = EMPTY_VALUE; maj_strong_low_time = 0; maj_strong_low_idx = -1;
               pending_strong_low_update = false; pending_strong_high_update = false;
            }
            ...
            if(ObjectFind(0, choch_name) < 0) {
               ...
               last_break_dir = 1; last_break_type = 2; 
               latest_break_up_time = time[i]; latest_break_up_idx = i; latest_break_up_level = maj_strong_high;
               ...
               if (ActiveHigh.time == maj_strong_high_time) ActiveHigh.isActive = false; ClearQueue(BOSDnQueue); 
               major_trend = 1; maj_extreme_high = high[i]; maj_extreme_high_time = time[i]; maj_extreme_high_idx = i;
         ...
         if (major_trend == 1 && pending_strong_low_update) {
             double min_swing_val = EMPTY_VALUE; datetime min_swing_time = 0; int min_swing_idx = -1;
             int end_idx = i;
             for(int k = i; k < rates_total; k++) {
                 if (time[k] <= last_strong_low_update_time) { end_idx = k; break; }
             }
             for(int k = i; k <= end_idx; k++) { 
                 if (majorSwingLow[k] != EMPTY_VALUE) {
                     if (min_swing_val == EMPTY_VALUE || majorSwingLow[k] < min_swing_val) {
                         min_swing_val = majorSwingLow[k]; min_swing_time = time[k]; min_swing_idx = k;
                     }
                 }
             }
             if (min_swing_val != EMPTY_VALUE) { 
                 double absolute_min_val = EMPTY_VALUE; datetime absolute_min_time = 0; int absolute_min_idx = -1;
                 for(int k = i; k <= min_swing_idx; k++) {
                     if (absolute_min_val == EMPTY_VALUE || low[k] < absolute_min_val) {
                         absolute_min_val = low[k]; absolute_min_time = time[k]; absolute_min_idx = k;
                     }
                 }
                 if (absolute_min_val != EMPTY_VALUE && absolute_min_val < min_swing_val) {
                     maj_strong_low = absolute_min_val; maj_strong_low_time = absolute_min_time; maj_strong_low_idx = absolute_min_idx;
                 } else {
                     maj_strong_low = min_swing_val; maj_strong_low_time = min_swing_time; maj_strong_low_idx = min_swing_idx;
                 }
                 last_strong_low_update_time = time[i]; 
                 pending_strong_low_update = false;
             }
         ...
         if (major_trend == -1 && pending_strong_high_update) {
             double max_swing_val = EMPTY_VALUE; datetime max_swing_time = 0; int max_swing_idx = -1;
             int end_idx = i;
             for(int k = i; k < rates_total; k++) {
                 if (time[k] <= last_strong_high_update_time) { end_idx = k; break; }
             }
             for(int k = i; k <= end_idx; k++) {
                 if (majorSwingHigh[k] != EMPTY_VALUE) {
                     if (max_swing_val == EMPTY_VALUE || majorSwingHigh[k] > max_swing_val) {
                         max_swing_val = majorSwingHigh[k]; max_swing_time = time[k]; max_swing_idx = k;
                     }
                 }
             }
             if (max_swing_val != EMPTY_VALUE) {
                 double absolute_max_val = EMPTY_VALUE; datetime absolute_max_time = 0; int absolute_max_idx = -1;
                 for(int k = i; k <= max_swing_idx; k++) {
                     if (absolute_max_val == EMPTY_VALUE || high[k] > absolute_max_val) {
                         absolute_max_val = high[k]; absolute_max_time = time[k]; absolute_max_idx = k;
                     }
                 }
                 if (absolute_max_val != EMPTY_VALUE && absolute_max_val > max_swing_val) {
                     maj_strong_high = absolute_max_val; maj_strong_high_time = absolute_max_time; maj_strong_high_idx = absolute_max_idx;
                 } else {
                     maj_strong_high = max_swing_val; maj_strong_high_time = max_swing_time; maj_strong_high_idx = max_swing_idx;
                 }
                 last_strong_high_update_time = time[i];
                 pending_strong_high_update = false;
             }
      ...
      if (i == 0) {
         ...
         if (ShowTrackingLines) { 
            ...
            if(major_trend == 1 && maj_confirmed_extreme_high != EMPTY_VALUE) {
               ...
               ObjectDelete(0, "IND_SMC_EXTREME_HIGH_ray"); ObjectDelete(0, "IND_SMC_EXTREME_HIGH_lbl");
            }
            ...
            if(major_trend == -1 && maj_confirmed_extreme_low != EMPTY_VALUE) {
               ...
               ObjectDelete(0, "IND_SMC_EXTREME_LOW_ray"); ObjectDelete(0, "IND_SMC_EXTREME_LOW_lbl");
            }
            ...
            if(major_trend == -1 && maj_strong_high != EMPTY_VALUE) {
               ...
               if(maj_strong_high_idx != -1) CreateStrongZone("IND_SMC_ACTIVE_STRONG_SELL_ZONE", maj_strong_high_idx, true, time, HAHigh, HALow, HAColor, rates_total);
            } else {
               ObjectDelete(0, "IND_SMC_STRONG_HIGH_ray"); ObjectDelete(0, "IND_SMC_STRONG_HIGH_lbl");
            ...
            if(major_trend == 1 && maj_strong_low != EMPTY_VALUE) {
               ...
               if(maj_strong_low_idx != -1) CreateStrongZone("IND_SMC_ACTIVE_STRONG_BUY_ZONE", maj_strong_low_idx, false, time, HAHigh, HALow, HAColor, rates_total);
            } else {
               ObjectDelete(0, "IND_SMC_STRONG_LOW_ray"); ObjectDelete(0, "IND_SMC_STRONG_LOW_lbl");
               ObjectDelete(0, "IND_SMC_ACTIVE_STRONG_BUY_ZONE");
            }
            
            if (MQLInfoInteger(MQL_TESTER)) ChartRedraw(0);
         } else { 
            ObjectDelete(0, "IND_SMC_TRACK_HIGH_ray"); ObjectDelete(0, "IND_SMC_TRACK_HIGH_lbl"); 
            ObjectDelete(0, "IND_SMC_TRACK_LOW_ray");  ObjectDelete(0, "IND_SMC_TRACK_LOW_lbl"); 
            ObjectDelete(0, "IND_SMC_EXTREME_HIGH_ray"); ObjectDelete(0, "IND_SMC_EXTREME_HIGH_lbl");
            ObjectDelete(0, "IND_SMC_EXTREME_LOW_ray");  ObjectDelete(0, "IND_SMC_EXTREME_LOW_lbl");
            ObjectDelete(0, "IND_SMC_STRONG_HIGH_ray");  ObjectDelete(0, "IND_SMC_STRONG_HIGH_lbl");
            ObjectDelete(0, "IND_SMC_STRONG_LOW_ray");   ObjectDelete(0, "IND_SMC_STRONG_LOW_lbl");
...
void CreateStrongZone(string name, int ext_idx, bool isSellZone, const datetime &time[], const double &ha_h[], const double &ha_l[], const double &ha_color[], int total) {
   if(ext_idx < 0 || ext_idx >= total) return;
   ...
   if(ObjectFind(0, name) >= 0) {
       ...
       if (MathAbs(old_zHigh - zHigh) < _Point && MathAbs(old_zLow - zLow) < _Point && old_tStart == tStart) return; 
       ObjectDelete(0, name);
   }
