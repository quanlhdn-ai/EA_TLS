...
// Tăng buffer lên 26 để chứa thêm 4 đường mZone tính toán ngầm cho Bot EA
...
int OnCalculate(const int rates_total, const int prev_calculated, const datetime &time[], const double &open[], const double &high[], const double &low[], const double &close[], const long &tick_volume[], const long &volume[], const int &spread[])
{
   ...
   for(int i = bos_limit; i >= 0; i--)
   {
            ...
            if (time[swingMajor_idx] > last_processed_high_time) {
                ...
                if (last_break_dir == -1 && last_break_type == 1 && time[swingMajor_idx] <= latest_break_down_time) {
                    ...
                    if (b_idx != -1) {
                        ...
                        double zEntry, zStop;
                        CreateZone(zone_name, swingMajor_idx, b_idx, true, latest_break_down_level, time, high, low, HAHigh, HALow, HAColor, rates_total, zEntry, zStop, true);
            ...
            if (time[swingMajor_idx] > last_processed_low_time) {
                ...
                if (last_break_dir == 1 && last_break_type == 1 && time[swingMajor_idx] <= latest_break_up_time) {
                    ...
                    if (b_idx != -1) {
                        ...
                        double zEntry, zStop;
                        CreateZone(zone_name, swingMajor_idx, b_idx, false, latest_break_up_level, time, high, low, HAHigh, HALow, HAColor, rates_total, zEntry, zStop, true);
      ...
      if(i > 0)
      {
         ...
         for(int j = ArraySize(MinorBuyZonesQueue) - 1; j >= 0; j--) { if(HAClose[i] < MinorBuyZonesQueue[j].stopPrice) { ObjectDelete(0, MinorBuyZonesQueue[j].name); for(int k = j; k < ArraySize(MinorBuyZonesQueue) - 1; k++) MinorBuyZonesQueue[k] = MinorBuyZonesQueue[k+1]; ArrayResize(MinorBuyZonesQueue, ArraySize(MinorBuyZonesQueue) - 1); } }
         for(int j = ArraySize(MinorSellZonesQueue) - 1; j >= 0; j--) { if(HAClose[i] > MinorSellZonesQueue[j].stopPrice) { ObjectDelete(0, MinorSellZonesQueue[j].name); for(int k = j; k < ArraySize(MinorSellZonesQueue) - 1; k++) MinorSellZonesQueue[k] = MinorSellZonesQueue[k+1]; ArrayResize(MinorSellZonesQueue, ArraySize(MinorSellZonesQueue) - 1); } }
         ...
         if (major_trend == 1 && maj_prot_low != EMPTY_VALUE && HAClose[i] < maj_prot_low) {
            ...
            if(ObjectFind(0, choch_name) < 0) {
               ...
               if(g_choch_up_name != "") { ObjectDelete(0, g_choch_up_name); ObjectDelete(0, g_choch_up_name + "_lbl"); g_choch_up_name = ""; }
               ...
               if(zone_anchor_idx != -1) {
                  ...
                  double zEntry, zStop;
                  CreateZone(zone_name, zone_anchor_idx, i, true, maj_prot_low, time, high, low, HAHigh, HALow, HAColor, rates_total, zEntry, zStop, true);
                  PushZone(SellZonesQueue, zone_name, zEntry, zStop, MaxZones);
               }
            ...
            if(ObjectFind(0, choch_name) < 0) {
               ...
               if(g_choch_dn_name != "") { ObjectDelete(0, g_choch_dn_name); ObjectDelete(0, g_choch_dn_name + "_lbl"); g_choch_dn_name = ""; }
               ...
               if(zone_anchor_idx != -1) {
                  ...
                  double zEntry, zStop;
                  CreateZone(zone_name, zone_anchor_idx, i, false, maj_prot_high, time, high, low, HAHigh, HALow, HAColor, rates_total, zEntry, zStop, true);
                  PushZone(BuyZonesQueue, zone_name, zEntry, zStop, MaxZones);
               }
            ...
            if (!is_major_choch_up) { 
                ...
                if(ObjectFind(0, bos_name) < 0) {
                   ...
                   if (origin_idx != -1) {
                      ...
                      double zEntry, zStop;
                      CreateZone(zone_name, origin_idx, i, false, ActiveHigh.price, time, high, low, HAHigh, HALow, HAColor, rates_total, zEntry, zStop, true);
                      PushZone(BuyZonesQueue, zone_name, zEntry, zStop, MaxZones);
                   }
                }
            }
            ...
            if (!is_major_choch_dn) { 
                ...
                if(ObjectFind(0, bos_name) < 0) {
                   ...
                   if (origin_idx != -1) {
                      ...
                      double zEntry, zStop;
                      CreateZone(zone_name, origin_idx, i, true, ActiveLow.price, time, high, low, HAHigh, HALow, HAColor, rates_total, zEntry, zStop, true);
                      PushZone(SellZonesQueue, zone_name, zEntry, zStop, MaxZones);
                   }
                }
            }
         ...
         if (minor_trend == 1 && min_prot_low != EMPTY_VALUE && HAClose[i] < min_prot_low) {
            ...
            if(ObjectFind(0, m_choch_name) < 0) {
               if (ShowMinorStructure) {
                   ...
                   if(g_minor_choch_up_name != "") { ObjectDelete(0, g_minor_choch_up_name); ObjectDelete(0, g_minor_choch_up_name + "_lbl"); g_minor_choch_up_name = ""; }
            ...
            if(ObjectFind(0, m_choch_name) < 0) {
               if (ShowMinorStructure) {
                   ...
                   if(g_minor_choch_dn_name != "") { ObjectDelete(0, g_minor_choch_dn_name); ObjectDelete(0, g_minor_choch_dn_name + "_lbl"); g_minor_choch_dn_name = ""; }
...
void CreateZone(string name, int swing_idx, int bos_idx, bool isSellZone, double broken_level, const datetime &time[], const double &h[], const double &l[], const double &ha_h[], const double &ha_l[], const double &ha_color[], int total, double &out_entry, double &out_stop, bool draw_graphic) {
   ...
   // Nếu tắt draw_graphic (khi ShowMinorStructure = false), hàm chỉ tính toán lấy giá trị Entry/SL rồi ngắt tại đây.
