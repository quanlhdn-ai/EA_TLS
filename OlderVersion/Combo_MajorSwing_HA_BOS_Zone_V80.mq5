...
int OnCalculate(const int rates_total, const int prev_calculated, const datetime &time[], const double &open[], const double &high[], const double &low[], const double &close[], const long &tick_volume[], const long &volume[], const int &spread[])
{
   ...
   for(int i = bos_limit; i >= 0; i--)
   {
      ...
      if(i > 0)
      {
         ...
         // [V80 FIX] LỚP BẢO VỆ CHOCH: Ngăn chặn tuyệt đối mọi lệnh vẽ đè
         ...
         if (minor_trend == 1 && min_prot_low != EMPTY_VALUE && HAClose[i] < min_prot_low) {
            if (!is_major_choch_dn) {
               string choch_name = "IND_SMC_MINOR_CHOCH_DN_" + IntegerToString((long)min_prot_low_time);
               if(ObjectFind(0, choch_name) < 0) {
            ...
            if (!is_major_choch_up) {
               string choch_name = "IND_SMC_MINOR_CHOCH_UP_" + IntegerToString((long)min_prot_high_time);
               if(ObjectFind(0, choch_name) < 0) {
         ...
         if (ActiveMinorHigh.isActive && HAClose[i] > ActiveMinorHigh.price) {
            if (!is_major_choch_up) {
                string bos_name = "IND_SMC_MINOR_BOS_UP_" + IntegerToString((long)ActiveMinorHigh.time);
                if(ObjectFind(0, bos_name) < 0) {
         ...
         if (ActiveMinorLow.isActive && HAClose[i] < ActiveMinorLow.price) {
            if (!is_major_choch_dn) {
                string bos_name = "IND_SMC_MINOR_BOS_DN_" + IntegerToString((long)ActiveMinorLow.time);
                if(ObjectFind(0, bos_name) < 0) {
      ...
      if (i == 0) {
         ...
         if (ShowTrackingLines) { CreateTrackingRayWithLabel("IND_SMC_TRACK_HIGH", g_high_time, g_high_lvl, TrackingLineColor, g_high_text, false, time[0]); CreateTrackingRayWithLabel("IND_SMC_TRACK_LOW", g_low_time, g_low_lvl, TrackingLineColor, g_low_text, true, time[0]); } else { ObjectDelete(0, "IND_SMC_TRACK_HIGH_ray"); ObjectDelete(0, "IND_SMC_TRACK_HIGH_lbl"); ObjectDelete(0, "IND_SMC_TRACK_LOW_ray");  ObjectDelete(0, "IND_SMC_TRACK_LOW_lbl"); }
      }
   }
   return(rates_total);
}
...
void OnChartEvent(const int id, const long &lparam, const double &dparam, const string &sparam) {
   if (id == CHARTEVENT_CHART_CHANGE) {
       if (ShowTrackingLines) { datetime t[]; if (CopyTime(_Symbol, _Period, 0, 1, t) > 0) { CreateTrackingRayWithLabel("IND_SMC_TRACK_HIGH", g_high_time, g_high_lvl, TrackingLineColor, g_high_text, false, t[0]); CreateTrackingRayWithLabel("IND_SMC_TRACK_LOW", g_low_time, g_low_lvl, TrackingLineColor, g_low_text, true, t[0]); ChartRedraw(); } }
   }
}
