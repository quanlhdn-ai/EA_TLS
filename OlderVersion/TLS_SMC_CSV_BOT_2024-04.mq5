//+------------------------------------------------------------------+
//|                                              TLS_SMC_Bot_V4.91   |
//|                    Focus: Ultimate Prop Shield & Perfect Radar   |
//|                    Status: Uncompressed & Beautifully Formatted  |
//+------------------------------------------------------------------+
...
// [NEW] Cài đặt bảo vệ Margin cho tài khoản Quỹ
input group "--- Prop Firm Protection ---"
input bool   Inp_UseMarginLimit     = true;  // Bật Lá chắn Ký quỹ (Margin)
input double Inp_MaxMarginPercent   = 38.0;  // % Hạn mức Margin tối đa cho phép
...
// ==================================================================
// CÁC HÀM BỔ TRỢ (Đã bung nén, format chuẩn)
// ==================================================================
...
bool LoadMatrixCSV() { 
    int handle = FileOpen("TLS_Matrix.csv", FILE_READ|FILE_TXT|FILE_ANSI|FILE_COMMON, 0, CP_UTF8); 
    if(handle == INVALID_HANDLE) { 
        Print("LỖI: Không tìm thấy TLS_Matrix.csv!"); 
        return false; 
    } 
    ...
    while(!FileIsEnding(handle)) { 
        ...
        string check_empty = line; 
        StringTrimLeft(check_empty); 
        StringTrimRight(check_empty); 
...
// ==================================================================
// HÀM TÍNH LOT & GLOBAL MARGIN SHIELD (BẢO VỆ TÀI KHOẢN QUỸ)
// ==================================================================
double CalculateLotSize(double sl_distance_points, double risk_multiplier) {
   ...
   if(Inp_UseMarginLimit) {
       ...
       if(remaining_margin_room <= 0) return 0; // Hết hạn mức Margin, không cho vào lệnh

       double margin_per_lot = 0;
       double ask_price = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
       if(!OrderCalcMargin(ORDER_TYPE_BUY, _Symbol, 1.0, ask_price, margin_per_lot)) {
           margin_per_lot = (SymbolInfoDouble(_Symbol, SYMBOL_TRADE_CONTRACT_SIZE) * ask_price) / AccountInfoInteger(ACCOUNT_LEVERAGE);
       }
...
void UpdateGatekeeperState() {
    ...
    if (SMC_HTF.current_buy_zone_entry > 0 && SMC_HTF.current_buy_zone_sl > 0) {
        double htf_top = SMC_HTF.current_buy_zone_entry;
        double htf_bot = SMC_HTF.current_buy_zone_sl;
        double w_pips = MathAbs(htf_top - htf_bot) / pip_size;
        double buffer_pips = MathMax((w_pips * HTF_Zone_Buffer_Pct / 100.0), Min_HTF_Buffer_Pips);
        double buffered_top = htf_top + (buffer_pips * pip_size);
        if (SymbolInfoDouble(_Symbol, SYMBOL_BID) <= buffered_top && SMC_HTF.current_buy_zone_sl != g_last_broken_buy_zone_sl) {
            g_gate_buy_open = true;
        }
    }

    if (SMC_HTF.current_sell_zone_entry > 0 && SMC_HTF.current_sell_zone_sl > 0) {
        double htf_bot = SMC_HTF.current_sell_zone_entry;
        double htf_top = SMC_HTF.current_sell_zone_sl;
        double w_pips = MathAbs(htf_top - htf_bot) / pip_size;
        double buffer_pips = MathMax((w_pips * HTF_Zone_Buffer_Pct / 100.0), Min_HTF_Buffer_Pips);
        double buffered_bot = htf_bot - (buffer_pips * pip_size);
        if (SymbolInfoDouble(_Symbol, SYMBOL_ASK) >= buffered_bot && SMC_HTF.current_sell_zone_sl != g_last_broken_sell_zone_sl) {
            g_gate_sell_open = true;
        }
    }
    ...
    if (CopyRates(_Symbol, _Period, 0, 2, r) >= 2) {
        ...
        if (g_gate_buy_open && SMC_HTF.current_buy_zone_entry > 0 && SMC_HTF.current_buy_zone_sl > 0) {
            double h = MathAbs(SMC_HTF.current_buy_zone_entry - SMC_HTF.current_buy_zone_sl);
            double kill_line = SMC_HTF.current_buy_zone_sl - (h * Zone_Break_Tolerance_Pct / 100.0);
            if (ha_low_1 < kill_line && ha_close_1 < SMC_HTF.current_buy_zone_sl) {
                g_gate_buy_open = false;
                g_last_broken_buy_zone_sl = SMC_HTF.current_buy_zone_sl; 
            }
        }

        if (g_gate_sell_open && SMC_HTF.current_sell_zone_entry > 0 && SMC_HTF.current_sell_zone_sl > 0) {
            double h = MathAbs(SMC_HTF.current_sell_zone_sl - SMC_HTF.current_sell_zone_entry);
            double kill_line = SMC_HTF.current_sell_zone_sl + (h * Zone_Break_Tolerance_Pct / 100.0);
            if (ha_high_1 > kill_line && ha_close_1 > SMC_HTF.current_sell_zone_sl) {
                g_gate_sell_open = false;
                g_last_broken_sell_zone_sl = SMC_HTF.current_sell_zone_sl;
            }
        }
    }
    
    // GỬI ẢNH MỞ CỔNG 1 LẦN
    if(g_gate_buy_open && !g_tg_buy_notified) {
       string msg = "🔓 <b>GATE OPENED: MỞ CỔNG MUA</b>\n━━━━━━━━━━━━━━━\n🔎 <b>Vị trí:</b> Giá chạm HTF Buy Zone\n⏳ Chờ xác nhận cấu trúc LTF để vào lệnh...";
       Radar.SendMessageWithPhoto(msg);
       g_tg_buy_notified = true;
    }
    if(!g_gate_buy_open) g_tg_buy_notified = false;

    if(g_gate_sell_open && !g_tg_sell_notified) {
       string msg = "🔓 <b>GATE OPENED: MỞ CỔNG BÁN</b>\n━━━━━━━━━━━━━━━\n🔎 <b>Vị trí:</b> Giá chạm HTF Sell Zone\n⏳ Chờ xác nhận cấu trúc LTF để vào lệnh...";
       Radar.SendMessageWithPhoto(msg);
       g_tg_sell_notified = true;
    }
...
double CalcTargetPrice(int signal, double entry_price, double sl_price, TMatrixRule &rule) {
    double tp = 0.0; double risk_val = MathAbs(entry_price - sl_price);
    double pip_size = GetPipSize(_Symbol);
...
void ManageTrades_Tick() {
   for(int i = 0; i < PositionsTotal(); i++) {
       ...
       if(magic >= BaseMagicNumber && magic < BaseMagicNumber + 1000) {
           bool found = false;
           ...
           if(!found) {
               ...
               g_trackers[size].partial_done = false; g_trackers[size].trail_r_watermark = 0.0;
               g_trackers[size].realized_pnl = 0; g_trackers[size].be_notified = false;
               double sl_dist = MathAbs(g_trackers[size].initial_open - g_trackers[size].initial_sl);
               g_trackers[size].initial_risk_money = sl_dist * g_trackers[size].initial_vol * (SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_VALUE) / SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_SIZE));
           }
       }
   }
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--) {
       ...
       if(PositionSelectByTicket(ticket)) {
               ...
               if (trigger_partial) {
                   ...
                   if (rule.Partial_Pct > 0) {
                       ...
                       if (close_vol >= SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN)) {
                           ...
                           if (close_vol >= PositionGetDouble(POSITION_VOLUME)) isPartialSuccess = trade.PositionClose(ticket); 
                           else isPartialSuccess = trade.PositionClosePartial(ticket, close_vol); 
                           
                           if(isPartialSuccess) {
                               Sleep(500); 
                               double pnl_added = AccountInfoDouble(ACCOUNT_BALANCE) - before_bal;
                               g_trackers[t_idx].realized_pnl += pnl_added;
                               string msg = "✂️ <b>PARTIAL CLOSE: CHỐT LỜI 1 PHẦN</b>\n━━━━━━━━━━━━━━━\n";
                               msg += "🎯 <b>Lý do:</b> Đạt mốc " + DoubleToString(rule.Partial_R, 1) + "R\n";
                               msg += "💰 <b>Lợi nhuận chốt:</b> +" + DoubleToString(pnl_added, 2) + "$\n";
                               msg += "📉 <b>Volume còn lại:</b> " + DoubleToString(PositionGetDouble(POSITION_VOLUME), 2) + " Lots";
           ...
           if (rule.BE_Trigger_R > 0 && current_R >= rule.BE_Trigger_R) {
               bool needs_be = false;
               if (type == POSITION_TYPE_BUY && current_sl < open_price) needs_be = true;
               if (type == POSITION_TYPE_SELL && current_sl > open_price) needs_be = true;
               if (needs_be) {
                   if(trade.PositionModify(ticket, NormalizeDouble(open_price, _Digits), PositionGetDouble(POSITION_TP))) {
                       ...
                       if(!g_trackers[t_idx].be_notified) {
                           string msg = "🛡️ <b>SL TRAILED: VỀ ĐIỂM HÒA VỐN</b>\n━━━━━━━━━━━━━━━\n";
                           msg += "🔒 <b>Trạng thái:</b> Đã dời SL về Entry (Risk Free)\n";
                           msg += "📈 <b>Lãi nổi:</b> +" + DoubleToString(PositionGetDouble(POSITION_PROFIT), 2) + "$";
                           Radar.SendMessage(msg);
           ...
           if (rule.Trail_Strategy != "NONE" && rule.Trail_Strategy != "") {
               ...
               if (rule.Trail_Strategy == "TRAIL_MINOR") {
                   double raw_sl = (type == POSITION_TYPE_BUY) ? SMC_LTF.current_minor_buy_zone_sl : SMC_LTF.current_minor_sell_zone_sl;
                   if (raw_sl > 0) new_sl = (type == POSITION_TYPE_BUY) ? (raw_sl - buffer_val) : (raw_sl + buffer_val);
               }
               else if (rule.Trail_Strategy == "TRAIL_MAJOR") {
                   double raw_sl = (type == POSITION_TYPE_BUY) ? SMC_LTF.current_buy_zone_sl : SMC_LTF.current_sell_zone_sl;
                   if (raw_sl > 0) new_sl = (type == POSITION_TYPE_BUY) ? (raw_sl - buffer_val) : (raw_sl + buffer_val);
               }
               else if (rule.Trail_Strategy == "TRAIL_STRUCT") {
                   double best_sl = current_sl; 
                   if (type == POSITION_TYPE_BUY) {
                       double p_low = SMC_LTF.current_maj_prot_low;
                       double cand_p = (p_low > 0 && p_low != EMPTY_VALUE) ? (p_low - buffer_val) : 0;
                       if (cand_p >= current_price) cand_p = 0;
                       if (cand_p > best_sl) best_sl = cand_p;
                       if (best_sl > current_sl) new_sl = best_sl;
                   } 
                   else if (type == POSITION_TYPE_SELL) {
                       double p_high = SMC_LTF.current_maj_prot_high;
                       double cand_p = (p_high > 0 && p_high != EMPTY_VALUE) ? (p_high + buffer_val) : 0;
                       if (cand_p <= current_price && cand_p > 0) cand_p = 0;
                       if (best_sl <= 0) best_sl = DBL_MAX;
                       if (cand_p > 0 && cand_p < best_sl) best_sl = cand_p;
                       if (best_sl < current_sl || current_sl <= 0) new_sl = best_sl;
                   }
               }
               else if (rule.Trail_Strategy == "TRAIL_R") {
                   if (current_R > g_trackers[t_idx].trail_r_watermark) {
                       g_trackers[t_idx].trail_r_watermark = current_R; 
                   }
                   if (g_trackers[t_idx].trail_r_watermark >= 1.0) {
                       double locked_R = g_trackers[t_idx].trail_r_watermark - 1.0; 
                       if (locked_R > 0) { 
                           new_sl = (type == POSITION_TYPE_BUY) ? (open_price + locked_R * initial_risk) : (open_price - locked_R * initial_risk);
                       }
                   }
               }
...
// BẮT SỰ KIỆN ĐÓNG LỆNH SL/TP TRỰC TIẾP
...
void ExecuteTradeLogic() {
   ...
   int action_type = 0; int rule_idx = -1; double risk_mult = 1.0; string loc_filter = "NONE";
   g_action_text = "Đứng ngoài (Không khớp CSV)"; 
   g_filter_text = ""; 
   ...
   for(int i=0; i<ArraySize(g_matrix_rules); i++) {
       if(g_matrix_rules[i].HTF_State == htf && g_matrix_rules[i].Maj_State == maj && g_matrix_rules[i].Min_State == min) {
           action_type = g_matrix_rules[i].Entry_Type;
           risk_mult = g_matrix_rules[i].Risk_Multiplier;
           loc_filter = g_matrix_rules[i].Location_Filter;
           g_action_text = g_matrix_rules[i].Dash_Note;
           rule_idx = i;
           is_found = true; break;
       }
   }

   ulong target_magic = BaseMagicNumber + rule_idx;
   trade.SetExpertMagicNumber(target_magic);

   if (action_type == 0) {
       for(int i = OrdersTotal() - 1; i >= 0; i--) {
           ulong ticket = OrderGetTicket(i); long magic = OrderGetInteger(ORDER_MAGIC);
           if(ticket > 0 && OrderGetString(ORDER_SYMBOL) == _Symbol && magic >= BaseMagicNumber && magic < BaseMagicNumber + 1000) trade.OrderDelete(ticket);
       }
       return;
   }
   ...
   if (loc_filter == "HTF_ZONE") {
       ...
       if (signal == 1 && g_gate_buy_open) is_passed = true;
       if (signal == -1 && g_gate_sell_open) is_passed = true;
       if (!is_passed) return; 
   }
   ...
   if (zone_width_pips > Max_Zone_SL_Pips) {
       double max_sl_val = Max_Zone_SL_Pips * pip_size;
       entry_zone = (signal == 1) ? (final_sl + max_sl_val) : (final_sl - max_sl_val);
       entry_zone = NormalizeDouble(entry_zone, _Digits);
       zone_width_val = max_sl_val;
   }
   if(zone_width_val <= 0) return; 

   if (signal == 1) { 
       double htf_sell_top = SMC_HTF.current_sell_zone_sl;
       double htf_sell_bot = SMC_HTF.current_sell_zone_entry;
       if (htf_sell_top > 0 && htf_sell_bot > 0) {
           double max_p = MathMax(htf_sell_top, htf_sell_bot);
           double min_p = MathMin(htf_sell_top, htf_sell_bot);
           if (entry_zone >= min_p) { g_filter_text = "Blocked by Buddha's Palm: Đang ngầm Mua trong lòng HTF Sell Zone!"; return; }
       }
   } 
   else if (signal == -1) { 
       double htf_buy_top = SMC_HTF.current_buy_zone_entry;
       double htf_buy_bot = SMC_HTF.current_buy_zone_sl;
       if (htf_buy_top > 0 && htf_buy_bot > 0) {
           double max_p = MathMax(htf_buy_top, htf_buy_bot);
           double min_p = MathMin(htf_buy_top, htf_buy_bot);
           if (entry_zone <= max_p) { g_filter_text = "Blocked by Buddha's Palm: Đang ngầm Bán trong lòng HTF Buy Zone!"; return; }
       }
   }

   if (Min_Reward_to_Risk_R > 0 && zone_width_val > 0) {
       double closest_obstacle = 0;
       if (signal == 1) { double obs_htf = SMC_HTF.current_sell_zone_entry; if (obs_htf > entry_zone) closest_obstacle = obs_htf; } 
       else if (signal == -1) { double obs_htf = SMC_HTF.current_buy_zone_entry; if (obs_htf > 0 && obs_htf < entry_zone) closest_obstacle = obs_htf; }

       double intended_target = CalcTargetPrice(signal, entry_zone, final_sl, g_matrix_rules[rule_idx]);
       double final_limit_price = 0;
       if (closest_obstacle > 0 && intended_target > 0) {
           if (signal == 1) final_limit_price = MathMin(closest_obstacle, intended_target);
           else if (signal == -1) final_limit_price = MathMax(closest_obstacle, intended_target);
       } else if (closest_obstacle > 0) { final_limit_price = closest_obstacle; } else if (intended_target > 0) { final_limit_price = intended_target; }

       if (final_limit_price > 0) {
           double potential_reward = MathAbs(final_limit_price - entry_zone);
           double current_rr = potential_reward / zone_width_val;
           if (current_rr < Min_Reward_to_Risk_R) { g_filter_text = "Blocked by Anti-Chop: RR = " + DoubleToString(current_rr, 1) + "R (Đích đến quá gần)"; return; } 
           else { g_filter_text = "Filter PASSED: RR = " + DoubleToString(current_rr, 1) + "R (Không gian thoáng)"; }
   ...
   for(int i = 0; i < PositionsTotal(); i++) {
       ...
       if(PositionSelectByTicket(ticket) && PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) >= BaseMagicNumber && PositionGetInteger(POSITION_MAGIC) < BaseMagicNumber + 1000) {
           ...
           double current_sl = PositionGetDouble(POSITION_SL);
           if (type == POSITION_TYPE_BUY && (current_sl < open_price || current_sl == 0)) risk_trades++;
           if (type == POSITION_TYPE_SELL && (current_sl > open_price || current_sl == 0)) risk_trades++;
       }
   }
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--) {
       ...
       if(ticket > 0 && OrderGetString(ORDER_SYMBOL) == _Symbol && magic >= BaseMagicNumber && magic < BaseMagicNumber + 1000) {
           ...
           double o_en = OrderGetDouble(ORDER_PRICE_OPEN);
           bool is_buy_limit = (type == ORDER_TYPE_BUY_LIMIT || type == ORDER_TYPE_BUY_STOP);
           bool is_sell_limit = (type == ORDER_TYPE_SELL_LIMIT || type == ORDER_TYPE_SELL_STOP);
   ...
   int dash_pos = StringFind(order_cmt, " - ");
   if(dash_pos > 0) order_cmt = StringSubstr(order_cmt, 0, dash_pos);

   double current_to_sl_val = MathAbs(current_price - final_sl);
   double current_to_sl_pips = current_to_sl_val / pip_size;
   bool success = false;
   ...
   if (signal == 1) {
       if (current_to_sl_pips <= Market_vs_Limit_Pips) { 
           ...
           if(lot <= 0) { g_filter_text = "Blocked: Hết hạn mức Margin (Shield Active)"; return; } // BẢO VỆ MARGIN
   ...
   if (success || trade.ResultRetcode() == 10009) {
       ...
       msg += "🏷️ <b>Rule:</b> " + g_matrix_rules[rule_idx].Dash_Note + "\n";
       msg += "📍 <b>Entry:</b> " + DoubleToString(entry_zone, _Digits) + "\n";
       msg += "🛡️ <b>SL:</b> " + DoubleToString(final_sl, _Digits) + " (" + DoubleToString(sl_pips, 1) + " pips)\n";
       msg += "🎯 <b>TP:</b> " + DoubleToString(lmt_tp, _Digits);
       Radar.SendMessageWithPhoto(msg); // <--- ĐÃ CHẮC CHẮN DÙNG HÀM CÓ ẢNH
   }
}
...
void UpdateDashboard() {
   ...
   ObjectSetInteger(0, "BOT_DASHBOARD_GK", OBJPROP_YDISTANCE, 105); ObjectSetInteger(0, "BOT_DASHBOARD_GK", OBJPROP_COLOR, clrGreen); ObjectSetInteger(0, "BOT_DASHBOARD_GK", OBJPROP_FONTSIZE, 8); ObjectSetString(0, "BOT_DASHBOARD_GK", OBJPROP_FONT, "Arial"); ObjectSetString(0, "BOT_DASHBOARD_GK", OBJPROP_TEXT, gk_text);
}

int OnInit() {
   Print("DA NẠP ENGINE V4.91 - PROP SHIELD & RADAR!");
   ...
   string msg = "🟢 <b>SYSTEM STARTED: TLS SMC BOT</b>\n━━━━━━━━━━━━━━━\n";
